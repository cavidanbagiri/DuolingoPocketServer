...
# For testing english
@router.post("/test-ai")
async def test_ai(word: str = "you"):
    """Test DeepSeek API connection with detailed logging"""
    ai_service = AIService()
    try:
        # Test the API call step by step
        prompt = ai_service._build_word_analysis_prompt(word)
        ...
        result["steps"].append("Making API request...")
        raw_response = await ai_service._make_deepseek_request(prompt)
        result["raw_response"] = raw_response

        if raw_response:
            result["steps"].append("API request successful")
            # Step 2: Parse response
            parsed_response = ai_service._parse_ai_response(word, raw_response)
            result["parsed_response"] = parsed_response.dict() if parsed_response else None
            result["steps"].append("Response parsed successfully")
        else:
            result["steps"].append("API request failed")
            result["error"] = "No response from API"

        result["status"] = "success" if raw_response else "failed"
        return result
...
async def check_config():
    ...
    return {
        "api_key_configured": os.getenv("DEEPSEEK_API_KEY"),
...
async def check_database(db: AsyncSession = Depends(get_db)):
    """Check database state and word counts"""
    from app.models.word_model import Word, Category, WordMeaning
    try:
        from sqlalchemy import func, select
        # from models import Word, WordMeaning, Category
        ...
        words_with_meanings = result.scalar()
        ...
        return {
            ...
            "words_with_meanings": words_with_meanings,
            "words_without_meanings": total_words - words_with_meanings,
...
# For creating Pos, definitions and categories
@router.post('/test-batch/{start_id}/{end_id}', status_code=200)
async def test_batch_words(
        start_id: int,
        end_id: int,
    ...
    """
    Test processing words within a specific ID range (e.g., 1-100)
    """
    try:
        repo = CreateMainStructureRepository(db)

        # Run in background to avoid timeout
        background_tasks.add_task(repo.process_word_batch, start_id, end_id)

        return {
            "message": f"Word processing started for IDs {start_id} to {end_id}",
            "status": "processing",
            "batch_size": end_id - start_id + 1
        }
    except Exception as ex:
        logger.error(f"Error starting batch processing: {str(ex)}")
        raise HTTPException(status_code=500, detail=str(ex))


@router.post('/process-all', status_code=200)
async def process_all_words(
        background_tasks: BackgroundTasks,
        db: AsyncSession = Depends(get_db)
) -> Dict[str, Any]:
    """
    Process all unprocessed words (full 10,000 words)
    """
    try:
        repo = CreateMainStructureRepository(db)

        background_tasks.add_task(repo.process_all_words)

        return {
            "message": "Full word processing started (all 10,000 words)",
            "status": "processing",
            "note": "This may take several hours. Check progress via /api/words/progress"
        }
    except Exception as ex:
        logger.error(f"Error starting full processing: {str(ex)}")
        raise HTTPException(status_code=500, detail=str(ex))


@router.get('/progress')
async def get_processing_progress(db: AsyncSession = Depends(get_db)):
    """
    Check processing progress statistics
    """
    try:
        from sqlalchemy import func, select
        ...
        # Processed words (with meanings)
        result = await db.execute(
            select(func.count(Word.id))
            .join(WordMeaning, Word.id == WordMeaning.word_id)
            .distinct()
        )
        processed_words = result.scalar()

        # Processing percentage
        percentage = (processed_words / total_words * 100) if total_words > 0 else 0

        return {
            "total_words": total_words,
            "processed_words": processed_words,
            "unprocessed_words": total_words - processed_words,
            "progress_percentage": round(percentage, 2),
            "status": "complete" if processed_words == total_words else "in_progress"
        }
    except Exception as e:
        return {"error": str(e), "status": "error"}
